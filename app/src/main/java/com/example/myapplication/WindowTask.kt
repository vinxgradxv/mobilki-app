...
@Composable
fun WindowTask(navController: NavHostController, token: String, clientId: String, facilityId: String, windowName:String) {

    // State variables to store user input
    val windowPerc = remember {
        mutableStateOf("")
    }
    ...
    Column(modifier = Modifier
        ...
        // Welcome message
        Text(text = "Настроить открытие окна", fontSize = 40.sp, color = Color.Black
        )

        OutlinedTextField(value = windowPerc.value, onValueChange = {
            windowPerc.value = it
        },
            leadingIcon = {
        ...
        OutlinedButton(onClick = {
            var response: CreateWindowTaskResponse
            scope.launch {
                try {
                    ...
                    response = apiRepo.sendCreateWindowTask(CreateWindowTaskRequest(windowPerc.value.toInt(), startTime, endTime, windowName), token)

                    navController.navigate(Routes.Windows.route
